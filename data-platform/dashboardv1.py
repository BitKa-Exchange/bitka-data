...
# --------------------------------------------------------
st.set_page_config(
    page_title="Bitka Command Center v1.1",
    page_icon="🚀",
    layout="wide",
)
...
st.title("🚀 Bitka Exchange — Command Center v1.1")
...
tickers = run_query(f"""
    SELECT * FROM fact_market_tickers
    WHERE event_time >= NOW() - INTERVAL '{window_sql}'
    ORDER BY event_time ASC;
""")

orders = run_query(f"""
    SELECT * FROM fact_orders_created
    WHERE event_time >= NOW() - INTERVAL '{window_sql}'
    ORDER BY event_time DESC;
""")

matches = run_query(f"""
    SELECT * FROM fact_matches_executed
    WHERE event_time >= NOW() - INTERVAL '{window_sql}'
    ORDER BY event_time DESC;
""")

logins = run_query(f"""
    SELECT * FROM dim_user_logins
    WHERE event_time >= NOW() - INTERVAL '{window_sql}'
    ORDER BY event_time DESC;
""")

deposits = run_query(f"""
    SELECT * FROM fact_deposits
    WHERE event_time >= NOW() - INTERVAL '{window_sql}'
    ORDER BY event_time DESC;
""")
...
# SYMBOL FILTER
if symbol != "ALL":
    tickers = tickers[tickers["symbol"] == symbol]
    orders = orders[orders["symbol"] == symbol]
    matches = matches[matches["symbol"] == symbol]
...
with colU:
    ...
    failed_logins = (
        logins["status"].astype(str).str.upper().eq("FAILED").sum()
    ...
    st.dataframe(
        logins[
            [c for c in logins.columns if c in ["event_time", "user_id", "ip_address", "location_geo", "status"]]
        ],
        hide_index=True,
...
with colF:
    ...
    st.dataframe(
        deposits[
            [c for c in deposits.columns if c in ["event_time", "user_id", "asset", "amount", "network"]]
        ],
        hide_index=True,
        use_container_width=True,
    )
